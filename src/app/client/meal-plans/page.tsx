import { getClientMealPlansPaginated } from "@/app/services/coaches/mealplans/getMealPlans";
import React from "react";
import Link from "next/link";
import { CalendarDays, User, ArrowRight, Clock, FileDown } from "lucide-react";
...
async function ClientMealPlans({ searchParams }: { searchParams?: SearchParams | Promise<SearchParams> }) {
  const params = (await Promise.resolve(searchParams)) ?? {};
  const rawPage = Number(params.page);
  const currentPage = Number.isFinite(rawPage) && rawPage > 0 ? Math.floor(rawPage) : 1;

  const { data: mealplans, totalCount } = await getClientMealPlansPaginated(currentPage, PAGE_SIZE);
  const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));
  const hasPrevious = currentPage > 1;
  const hasNext = currentPage < totalPages;
  const pageItems = getVisiblePageItems(currentPage, totalPages);

  const getPageHref = (page: number) => (page <= 1 ? "/client/meal-plans" : `/client/meal-plans?page=${page}`);
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {mealplans.length > 0 ? (
        ...
        <div className="text-center py-24 bg-white rounded-xl border border-gray-100">
          ...
          <p className="text-gray-400 mb-6">Je coach heeft nog geen weekplannen aangemaakt voor jou.</p>
        </div>
      )}
    </div>
  );
