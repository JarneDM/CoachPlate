import { getClientPlans } from '@/app/services/clients/plans/plans';
...
async function ClientPlans({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [plans, client] = await Promise.all([getClientPlans(id), getClientById(id)]);
      ...
      {plans.length > 0 ? (
        ...
        <div className="text-center py-24 bg-white rounded-xl border border-gray-100">
          ...
          <div className="items-center justify-center flex">
            <Button href={`/meal-plans/new?client=${id}`} label="Eerste weekplan aanmaken" icon={<Plus size={14} />} width="max-w-[300px] w-auto" />
          </div>
        </div>
      )}
    </div>
