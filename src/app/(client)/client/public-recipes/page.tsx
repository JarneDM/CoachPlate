import { getPublicRecipePaginated } from "@/app/services/recipes/recipes";
...
export default async function PublicRecipesPage({ searchParams }: { searchParams?: SearchParams | Promise<SearchParams> }) {
  ...
  const getPageHref = (page: number) => (page <= 1 ? "/client/public-recipes" : `/client/public-recipes?page=${page}`);

  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-8 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Publieke recepten</h1>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {recipes.map((recipe) => (
              <div key={recipe.id} className="bg-white rounded-xl border border-gray-100 p-5 hover:border-green-200 transition-colors">
                ...
                <Link href={`/client/public-recipes/${recipe.id}`} className="text-xs text-green-600 font-medium hover:underline">
                  Bekijk recept →
                </Link>
              </div>
            ))}
